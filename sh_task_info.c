...
//	printk("%s",task->comm);
	...
	sprintf(msgexec_start,"%lu",se->exec_start);
	sprintf(msgsum_exec_runtime,"%lu",se->sum_exec_runtime);
	sprintf(msgvruntime,"%lu",se->vruntime);
	sprintf(msgprev_sum_exec_runtime,"%lu",se->prev_sum_exec_runtime);
	sprintf(msgnr_migrations,"%lu",se->nr_migrations);
...
//	printk("value of cpu is %d",task->cpu);
	...
	printk(KERN_ALERT"value of exec_start is %lu\n",se->exec_start);
	printk(KERN_ALERT"value of sum_exec_runtime is %lu\n",se->sum_exec_runtime);
	printk(KERN_ALERT"value of vruntime is %lu\n",se->vruntime);
	printk(KERN_ALERT"value of prev_sum_exec_runtime is %lu\n",se->prev_sum_exec_runtime);
	printk(KERN_ALERT"nr_migrations = %lu\n",se->nr_migrations);
	printk(KERN_ALERT"sp0 is %lu\n",thread->sp0);
	printk(KERN_ALERT"sp is %lu\n",thread->sp);
